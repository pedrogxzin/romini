...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (isNaN(Value) || Value < 0 || Value > 100_000_000_000_000_000) return client.sendReply(message, {
            content: `${client.config.emojis.error} ${message.author}, digite um valor número maior ou igual a **0 Flocos** para enviar.`
        })

        //CANAL FIXO
        const channel = client.channels.cache.get('1236467201486229654') || client.channels.cache.get('1236467201486229654')
        ...
        const Message = await client.sendReply(message, {
            content: `${User} definiu seu saldo como ${client.config.emojis.money} **${Value.toLocaleString()} Flocos**.`
        })
    }
}